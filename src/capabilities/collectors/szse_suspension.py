...
def collect(limit: int = 20) -> List[Dict[str, str]]:
    payload = json.loads(fetch_text(f"{SZSE_LIST_URL}?random=0.1&pageSize={max(limit, 20)}&pageNum=1&plateCode=szse"))
    ...
    for company in payload.get("data", []):
        ...
        for item in company.get("announList", []):
            ...
            url = f"https://disc.static.szse.cn/download{attach_path}" if attach_path else ""
            rows.append(
                {
                    "source": "深交所/停复牌公告",
                    "title": title,
