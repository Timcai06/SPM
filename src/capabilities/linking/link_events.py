...
def l1_to_category(l1: str) -> str:
    ...
    code = (
        ...
        if len(text) >= 3 and text[:1].isalpha() and text[1:3].isdigit()
        else text
    )
    code = (
        code[:3]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate minimal event-company links."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=120,
        help="Max seconds to wait for DB write lock.",
    )
    parser.add_argument(
        "--canonical-map",
        default=str(CANONICAL_MAP_PATH),
        help="Optional event canonical map CSV.",
    )
    parser.add_argument(
        "--progress-every",
        type=int,
        default=250,
        help="Print progress every N clustered events.",
    )
...
def score_link(event: dict, company: dict) -> tuple[float, dict]:
    ...
    text_hits = [
        kw
        for kw in EVENT_KEYWORDS.get(event["industry_type"], EVENT_KEYWORDS["其他"])
        if kw and kw in company_text
    ]
    event_hits = [
        kw
        for kw in EVENT_KEYWORDS.get(event["industry_type"], EVENT_KEYWORDS["其他"])
        if kw and kw in event_text
    ]
    ...
    title_name_match = (
        1.0 if company["company_name"] and company["company_name"] in raw_title else 0.0
    )
    ...
    title_keyword_hits = [
        kw
        for kw in EVENT_KEYWORDS.get(event["industry_type"], EVENT_KEYWORDS["其他"])
        if kw and kw in raw_title
    ]
    title_keyword_match = min(len(title_keyword_hits) / 2.0, 1.0)
    chain_position = (
        1.0
        if title_name_match or direct_name_match
        else (
            0.9 if direct_symbol_match else (0.7 if text_hits or concept_hits else 0.2)
        )
    )
...
def build_cluster_events(
    events: list[dict], event_to_cluster: dict[str, dict]
) -> list[dict]:
    ...
    for event in events:
        ...
        members = [
            event_by_event_id[event_id]
            for event_id in cluster["member_event_ids"]
            if event_id in event_by_event_id
        ]
        representative = event_by_event_id.get(
            cluster["representative_event_id"], members[0] if members else event
        )
        raw_title = " | ".join(
            dict.fromkeys(
                member["raw_title"] for member in members if member.get("raw_title")
            )
        )
        raw_content = " | ".join(
            dict.fromkeys(
                member["raw_content"] for member in members if member.get("raw_content")
            )
        )
        raw_symbol = " | ".join(
            dict.fromkeys(
                (member.get("raw_symbol") or "").strip()
                for member in members
                if (member.get("raw_symbol") or "").strip()
            )
        )
        event_summary = " | ".join(
            dict.fromkeys(
                member["event_summary"]
                for member in members
                if member.get("event_summary")
            )
        )
        built.append(
            {
                **representative,
                ...
                or representative.get("event_summary")
                or "",
            }
        )
    return built
...
def main() -> None:
    ...
    with write_guard(
        ...
        with psycopg.connect(
            dsn_for(args.db), row_factory=psycopg.rows.dict_row
        ) as conn:
            with conn.cursor() as cur:
                ...
                for event_idx, event in enumerate(cluster_events, start=1):
                    ...
                    for company in companies:
                        ...
                        if (
                            is_generic_event(event)
                            and event.get("industry_type") == "其他"
                        ):
                    ...
                    if is_generic_event(event) and all(
                        item[2]["direct_symbol_score"] < 1
                        and item[2]["direct_name_score"] < 1
                        for item in scored[:1]
                    ):
                        limit_k = min(
                            args.top_k, 2 if event.get("industry_type") == "其他" else 4
                        )
                    else:
                        limit_k = args.top_k
                    for final_score, company, details in scored[:limit_k]:
                        link_type = (
                            "direct_match"
                            if details["direct_symbol_score"] >= 1
                            or details["direct_name_score"] >= 1
                            else (
                                "industry_match"
                                if details["industry_match_score"] >= 1
                                else "candidate"
                            )
                        )
                        for structured_event_id in event["member_ids"]:
                            ...
                            current_keys.append(
                                (sid, int(company["id"]), str(link_type))
                            )
                    if args.progress_every > 0 and (
                        event_idx == 1
                        or event_idx % args.progress_every == 0
