#!/usr/bin/env python3
"""Import A-share company universe from AKShare directly into PostgreSQL."""
        ...
        rows.append(
            {
                ...
                "business_scope": name,
                "core_products": "其他",
                "concept_tags": json.dumps(["其他", name], ensure_ascii=False),
            }
        )
            ...
            with conn.cursor() as cur:
                cur.executemany(
                    ...
                    SET company_name = CASE
                        ...
                        END,
                        industry_l1 = CASE
                            WHEN companies.industry_l1 IS NULL OR companies.industry_l1 = '' THEN EXCLUDED.industry_l1
                            ELSE companies.industry_l1
                        END,
                        industry_l2 = CASE
                            WHEN companies.industry_l2 IS NULL OR companies.industry_l2 = '' THEN EXCLUDED.industry_l2
                            ELSE companies.industry_l2
                        END,
                        business_scope = CASE
                            WHEN companies.business_scope IS NULL OR companies.business_scope = '' THEN EXCLUDED.business_scope
                            ELSE companies.business_scope
                        END,
                        core_products = CASE
                            WHEN companies.core_products IS NULL OR companies.core_products = '' THEN EXCLUDED.core_products
                            ELSE companies.core_products
                        END,
                        concept_tags = CASE
                            WHEN companies.concept_tags = '[]'::jsonb THEN EXCLUDED.concept_tags
...
def main() -> None:
    args = parse_args()
    rows = load_akshare_rows(offset=args.offset, max_symbols=args.max_symbols)
    if not rows:
        raise RuntimeError("AKShare returned zero A-share companies.")
    upsert_companies(args.db, rows, lock_timeout_sec=args.lock_timeout_sec)
