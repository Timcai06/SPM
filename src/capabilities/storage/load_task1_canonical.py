...
CREATE_SQL = """
CREATE TABLE IF NOT EXISTS canonical_event_clusters (
...
CREATE INDEX IF NOT EXISTS idx_canonical_event_clusters_date_start
    ON canonical_event_clusters (date_start DESC);

CREATE INDEX IF NOT EXISTS idx_canonical_event_clusters_subject_type
    ON canonical_event_clusters (event_subject_type);

CREATE TABLE IF NOT EXISTS canonical_event_memberships (
    id BIGSERIAL PRIMARY KEY,
    structured_event_id BIGINT NOT NULL UNIQUE REFERENCES structured_events(id) ON DELETE CASCADE,
    canonical_event_id TEXT NOT NULL REFERENCES canonical_event_clusters(canonical_event_id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_canonical_event_memberships_canonical_event
    ON canonical_event_memberships (canonical_event_id);
"""
...
def load_canonical_events(cur: psycopg.Cursor, rows: list[dict[str, str]]) -> int:
    sql = """
    INSERT INTO canonical_event_clusters (
...
def load_event_links(cur: psycopg.Cursor, rows: list[dict[str, str]]) -> int:
    ...
    cur.execute("DELETE FROM canonical_event_memberships")
    sql = """
    INSERT INTO canonical_event_memberships (
...
def run_loading_pipeline(db: str, canonical_event_rows: list[dict[str, str]] = None, canonical_link_rows: list[dict[str, str]] = None, lock_timeout_sec: int = 120, quiet: bool = False) -> None:
    ...
    with write_guard(
        db_name=db,
        required_tables=["structured_events", "canonical_event_clusters", "canonical_event_memberships"],
    ...
    if not quiet:
        print(f"Upserted {upserted} canonical_event_clusters into {db}")
        print(f"Loaded {linked} canonical_event_memberships into {db}")
