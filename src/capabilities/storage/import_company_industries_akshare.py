...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backfill company industry fields from AKShare/EastMoney boards.")
    parser.add_argument("--db", default=DEFAULT_DB)
    parser.add_argument("--max-industries", type=int, default=0, help="Optional limit for staged runs.")
...
def load_industry_names(offset: int, max_industries: int) -> list[str]:
    ...
    name_col = "板块名称"
    names = [str(value).strip() for value in df[name_col].tolist() if str(value).strip()]
    start = max(offset, 0)
...
def update_companies(db_name: str, by_code: dict[str, list[str]], lock_timeout_sec: int) -> int:
    ...
    for ts_code, industries in by_code.items():
        ...
        industry_l1 = industries[0]
        tags = json.dumps(industries[:8], ensure_ascii=False)
        rows.append({"ts_code": ts_code, "industry_l1": industry_l1, "industry_l2": industry_l2, "concept_tags": tags})
    if not rows:
        return 0
...
def main() -> None:
    args = parse_args()
    industry_names = load_industry_names(offset=args.offset, max_industries=args.max_industries)
    print(f"[industry-backfill] industries={len(industry_names)} offset={args.offset}", flush=True)
    by_code = collect_members(industry_names, sleep_sec=args.sleep_sec, progress_every=args.progress_every)
