...
def run_transactional_sql(db_name: str, statements: list[str]) -> None:
...
def copy_csv_to_table(
    ...
    buffer.seek(0)
    copy_sql = f"COPY {table_name} ({', '.join(fieldnames)}) FROM STDIN WITH (FORMAT csv, HEADER true)"
...
def load_stage_rows(
    ...
    candidate_rows = build_candidate_stage_rows(raw_candidates)
    structured_rows = build_structured_stage_rows(structured_events)
    conn = psycopg.connect(dsn_for(db_name))
    try:
        with conn.cursor() as cur:
            copy_csv_to_table(
