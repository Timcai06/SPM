...
CREATE INDEX IF NOT EXISTS idx_company_stats_ts_code_trade_date
    ON company_stats (ts_code, trade_date DESC);
...
CREATE TABLE IF NOT EXISTS model_non_event_samples (
    ...
    label_up_w3 BOOLEAN,
    label_up_w5 BOOLEAN,
    label_source TEXT NOT NULL DEFAULT 'company_stats',
    created_at TIMESTAMP NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMP NOT NULL DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_model_non_event_samples_sample_date
    ON model_non_event_samples (sample_date DESC);
